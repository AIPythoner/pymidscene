...
使用 AI 执行复杂任务，完整的 plan-execute-replan 循环。
...
2. 执行 AI 规划的动作序列（支持 Tap、Input、Hover、Scroll、KeyboardPress、Sleep、Assert）
3. 如果 AI 返回 `shouldContinuePlanning=true`，重新截图并 replan
4. 循环直到任务完成或达到最大重规划次数（10 次）

#### 缓存机制

- 首次执行：AI 规划 → 执行 → 将动作序列保存为 YAML workflow 缓存
- 后续执行：直接从缓存读取动作序列并回放（跳过 AI 调用）

#### 示例

```python
# 执行复杂交互（AI 自动处理滚动、点击等）
await agent.ai_action("点击自主声明选项的下拉选择框")

# AI 会自动处理元素不在视口中的情况
# 发现目标不可见 → 规划 Scroll → 滚动后重新截图 → 找到元素后操作
await agent.ai_action("选择下拉菜单中的 虚拟演绎 选项")

# 多步骤任务
await agent.ai_action("在搜索框中输入 Python 并点击搜索按钮")
```

#### 与 ai_click 的区别

| 特性 | `ai_click` | `ai_action` |
|------|-----------|-------------|
| 定位方式 | 直接 AI 定位 | AI 规划动作序列 |
| 滚动处理 | 固定滚动重试 | AI 自主决定是否滚动 |
| 动作类型 | 仅点击 | 支持点击、输入、滚动、按键等 |
| 适用场景 | 简单点击 | 复杂交互流程 |
...
| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `assertion` | `str` | - | 断言条件描述 |
| `timeout` | `float` | `30` | 超时时间（秒） |
| `interval` | `float` | `2` | 轮询间隔（秒） |
...
# 等待元素出现
await agent.ai_wait_for("标题输入框已出现", timeout=15)

# 自定义轮询间隔
await agent.ai_wait_for("上传进度达到 100%", timeout=60, interval=3)
```
...
| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `direction` | `str` | `"down"` | 滚动方向（up/down/left/right） |
| `distance` | `int` | `500` | 滚动距离（像素） |
| `scroll_type` | `str` | `"singleAction"` | 滚动类型 |
| `locate_prompt` | `Optional[str]` | `None` | 可选的元素描述，在该元素上滚动 |

#### 滚动类型

| 类型 | 说明 |
|------|------|
| `singleAction` | 单次滚动指定距离 |
...
```python
# 向下滚动
await agent.ai_scroll("down", 500)

# 滚动到页面底部
await agent.ai_scroll(scroll_type="scrollToBottom")

# 在特定元素上滚动
await agent.ai_scroll("down", 300, locate_prompt="商品列表区域")
```
