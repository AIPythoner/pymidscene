...
友好应用名 → iOS Bundle ID 的映射.

对应 JS 版: packages/ios/src/appNameMapping.ts
...
def resolve_bundle_id(
    ...
    """
    在映射中查找 app_name 对应的 bundle ID (大小写/空格不敏感).

    对应 JS `IOSDevice.resolveBundleId`.
    """
    if not app_name:
        return None
...
__all__ = ["DEFAULT_APP_NAME_MAPPING", "resolve_bundle_id"]
