...
def _chinese_plan_prompt() -> str:
    # 完整移植 JS getAutoGLMChinesePlanPrompt (auto-glm/prompt.ts:145-205):
    # 19 条规则 + 完整动作说明. {{think}}/{{action}} 是字面占位符(转义).
    # 坐标网格说明保持 (0,0)-(999,999) 与 JS 一致(代码侧除以 1000).
    return f"""
...
def get_auto_glm_locate_prompt(model_family: Optional[str]) -> str:
    """完整移植 JS getAutoGLMLocatePrompt (auto-glm/prompt.ts:220-278)."""
    if model_family == "auto-glm-multilingual":
        return f"""
The current date: {_multilingual_date()}
