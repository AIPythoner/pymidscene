...
Maps friendly app names to their Android package names.

对应 JS 版: packages/android/src/appNameMapping.ts
...
DEFAULT_APP_NAME_MAPPING: dict[str, str] = {
    "微信": "com.tencent.mm",
...
def resolve_package_name(
    ...
    """
    在映射中查找 app_name 对应的 package name（大小写与空格不敏感）.

    对应 JS `AndroidDevice.resolvePackageName`.

    Returns:
        命中的 package name；未命中返回 None.
    """
    if not app_name:
        return None
    table = mapping if mapping is not None else DEFAULT_APP_NAME_MAPPING
    target = _normalize_for_comparison(app_name)
    for key, value in table.items():
        if _normalize_for_comparison(key) == target:
            return value
    return None


__all__ = ["DEFAULT_APP_NAME_MAPPING", "resolve_package_name"]
